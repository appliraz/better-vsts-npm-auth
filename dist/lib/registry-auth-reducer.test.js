...
function generateTests(name, useLegacyUri) {
    describe(name, () => {
        let originalEnv;
        ...
        describe("has a static method filterUniqueVstsRegistries which", () => {
            test("given an array containing duplicate registries, removes the redundant entries", () => {
                ...
                let testRegistries = [
                    new npm_1.Registry(registry_bar_mirrorFeed),
                    new npm_1.Registry(registry_foo_npmMirrorFeed),
                    new npm_1.Registry(registry_bar_mirrorFeed),
                    new npm_1.Registry(registry_foo_customFeed),
                    new npm_1.Registry(registry_bar_mirrorFeed),
                    new npm_1.Registry(registry_foo_customFeed),
                ];
                ...
                ].forEach((r) => expect(filteredRegistries).toContainEqual(new npm_1.Registry(r)));
                expect.assertions(4);
            });
            test("given an array containing non-visualstudio.com hosted registries, removes them", () => {
                ...
                let testRegistries = [
                    new npm_1.Registry(registry_someOtherFeed),
                    new npm_1.Registry(registry_vstsFeed),
                ];
                let filteredRegistries = RegistryAuthReducer.filterUniqueVstsRegistries(testRegistries);
                expect(filteredRegistries).toHaveLength(1);
                expect(filteredRegistries).toContainEqual(new npm_1.Registry(registry_vstsFeed));
            });
        });
        describe("has a static method shardRegistriesByCollection which", () => {
            ...
            test("given input with a mixture of internal and external registries, sorts them into the proper result", () => {
                let sameCollectionEntries = [
                    new npm_1.Registry(useLegacyUri
                ...
                expect(result).toHaveProperty("differentCollection", otherCollectionEntries);
        ...
        describe("has a static method authenticateRegistries which", () => {
            it("gets a user auth token and uses it for all registries when no lab token is available", () => {
                ...
                let result = RegistryAuthReducer.authenticateRegistries(...registries);
            ...
            describe("when a lab token is available,", () => {
                ...
                test("applies the lab token to all registries hosted in the same VSTS project collection", () => __awaiter(this, void 0, void 0, function* () {
                    let result = yield RegistryAuthReducer.authenticateRegistries(...sameRegistries, ...differentRegistries);
                ...
                test("does not authenticate the any registries which are not in the same project colleciton", () => __awaiter(this, void 0, void 0, function* () {
                    let result = yield RegistryAuthReducer.authenticateRegistries(...differentRegistries);
...
//# sourceMappingURL=registry-auth-reducer.test.js.map